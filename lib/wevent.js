...
class Wevent {
  ...
  _getEventListeners(object, create) {
    if (!(object instanceof Object)) {
      throw new Error(
        "Can only bind event listener to instance of \"Object\"."
      );
    }
  ...
  /**
   * Recursive function that emits a single event listener.
   * @private
   ...
   * @return {Promise.<number>} Promise that resolves to event listener 
   * count that received the emit.
   */
  _emitEventListener(
    resolve,
    reject,
  ...
  off(object, eventListener) {
    ...
    if (eventListeners) {
      ...
      if (removeEventListenerObject) {
        this._eventObjects.delete(object);
      }
    }
   ...
   * resolves into count of event listeners emits.
   */
  emit(object) {
  ...
  count(object, eventListener) {
    ...
    let count = 0;
    for (eventListener of eventListeners.values()) {
      count += eventListener.count;
    }
    return count;
  }
}
